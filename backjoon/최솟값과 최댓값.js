...
class SegmentTree {
  constructor(nums) {
    ...
    this.tree = Array.from({ length: 2 ** (this.depth + 1) }, () => [
      Infinity,
      -Infinity,
    ]); // 세그먼트 트리 배열
    this.init();
  ...
  init() {
    ...
    for (let i = 0; i < this.nums.length; i++) {
      this.tree[i + leftIdx + 1] = [this.nums[i], this.nums[i]];
    }
  ...
  merge(leftValue, rightValue) {
    const [leftMin, leftMax] = leftValue;
    const [rightMin, rightMax] = rightValue;
  ...
  search(leftIdx, rightIdx) {
    ...
    if (leftIdx % 2 === 1) {
      if (this.tree[leftIdx]) {
        this.minMax = this.merge(this.minMax, this.tree[leftIdx]);
      }
    }
    if (rightIdx % 2 === 0) {
      if (this.tree[rightIdx]) {
        // console.log(this.tree[rightIdx]);
        this.minMax = this.merge(this.minMax, this.tree[rightIdx]);
      }
    }
...
function solution() {
  ...
  for (let qIdx = N + 1; qIdx <= N + M; qIdx++) {
    result.push(
      segTree.queryRange(...input[qIdx].split(" ").map(Number)).join(" ")
    );
  }
